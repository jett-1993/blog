...
def expert_panel_review(article_data, language="Korean"):
    ...
    [검증 및 수정 포인트]
    1. IT/경제/사회 전문가: 팩트 오류 검증, 논리의 깊이 추가, 너무 가벼운 내용 제거, 통찰력 추가.
    2. SEO/애드센스 전문가: 체류시간을 늘리기 위한 훅(Hook) 강화, 자연스러운 키워드 배치, 가독성 향상. 사람 냄새나는 톤(Jett입니다 등) 유지.
    3. 그래픽 디자이너: 썸네일 프롬프트가 16:9 와이드 비율('16:9 wide aspect ratio')인지, 고품질인지, 텍스트가 없는지('no text', 'no logo') 점검 및 강화.
